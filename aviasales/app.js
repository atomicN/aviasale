...
const selectCity = (event, input, list) => {
    const target = event.target;
        if(list.textContent === "Некорректное название города" ){
...
formSearch.addEventListener('submit', (event) => {
    ...
    const modal = (input, list) => {
        const li = document.createElement('li');
        li.classList.add('dropdown__warn');
        li.textContent = "Некорректное название города";
        list.append(li);
    ...
    if (FormData.from && FormData.to) {
        ...
        modal(inputCitiesFrom,dropdownCitiesFrom); 

    } else if (!FormData.to && FormData.from){
        modal(inputCitiesTo, dropdownCitiesTo);
